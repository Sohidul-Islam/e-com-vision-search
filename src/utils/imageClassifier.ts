...
export async function loadModel(): Promise<void> {
  if (!model && typeof window !== 'undefined') {
    ...
    if (!window.tf || !window.mobilenet) {
      console.error('[ImageClassifier] TensorFlow.js or MobileNet not available');
    ...
    console.log('[ImageClassifier] TensorFlow.js backend:', window.tf.getBackend());
 ...
 */
export async function classifyImage(imageElement: HTMLImageElement): Promise<Prediction[]> {
